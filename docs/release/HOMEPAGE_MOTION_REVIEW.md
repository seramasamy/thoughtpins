...
| Site motion | Restore the original fuller floating memory network: warm glowing hubs, violet/ember connections, subtle traveling signals, and a shared Pause motion control. Marquee copies include their trailing gap. | Preserve the expressive connected background while keeping text readable. Seeded positions and a spanning tree retain stable connections on resize; links are computed outside the animation loop. Drawing stays capped at 30 Hz with 64 phone or 112 desktop/tablet nodes. Both homepages follow the session choice, live Reduce Motion, page visibility and viewport intersection. |
