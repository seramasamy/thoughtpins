# Running Thought Pins

Everything past the sixty-second local start in the [README](../../README.md): full local
setup, the API surface, the Obsidian vault contract, private adapters, and the checks to run
before a shared deployment.

For deploy, rollback, backup, and restore procedures see
[PRODUCTION_RUNBOOK.md](PRODUCTION_RUNBOOK.md). For the build order see
