...
| PDF and text notes | Extract embedded PDF text from up to 100 pages. Scanned pages without text are read with local OCR (Tesseract), up to 20 pages and 40 seconds per upload; embedded text is never replaced by OCR, turned pages are read upright, and pages stored as strips are reassembled. Page limits, OCR'd pages and pages still unread are reported with the reason. Handwriting and page layout fidelity are not established. Files are limited to 25 MB. |
