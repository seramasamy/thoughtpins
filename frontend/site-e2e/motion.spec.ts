...
async function instrumentCanvas(page: Page) {
  await page.addInitScript(() => {
    ...
    CanvasRenderingContext2D.prototype.clearRect = function (...args) {
      if (this.canvas.hasAttribute("data-constellation")) {
...
const draws = (page: Page) => page.locator("[data-constellation]").getAttribute("data-draws");
...
test("blocked storage and unavailable canvas do not break the page", async ({ page }) => {
  await page.addInitScript(() => {
