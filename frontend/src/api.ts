...
// Re-exported so callers keep importing everything from ./api.
export { ApiError, configureApiSession };
...
export const api = {
  ...
  requestInvite(token: string, note: string) {
    return request<InviteRequestResponse>("/v1/invites/request", {
      method: "POST",
      token,
      body: { note: note || null },
    });
