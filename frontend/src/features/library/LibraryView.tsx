...
import { UPLOAD_ACCEPT, UPLOAD_HELP, uploadOutcome } from "../../core/upload";
...
export function LibraryView({ token, run }: ScreenProps) {
    ...
    await perform(async () => {
      if (addMode === "file") {
        ...
        if (result) {
          ...
          setLast(null);
          if (uploadOutcome(result).saved && uploadOutcome(result).tone !== "error") clearComposer();
          void load();
        }
        return;
      }
  ...
  const clearComposer = () => {
    ...
    setAuthor(current => current === author ? "" : current);
      ...
      <div className="library-layout">
        <form className={`panel library-add mode-${addMode}`} onSubmit={submit}>
          ...
          <div className="form-stack library-fields">
            ...
            {addMode === "file" && (
              <>
                ...
                <label className="file-picker">File Upload<input type="file" accept={UPLOAD_ACCEPT} onChange={(event) => setFile(event.target.files?.[0] || null)} required disabled={pending} /><span>{file ? file.name : "Choose a document, image, or audio note"}</span></label>
                <p className="inline-help">{UPLOAD_HELP}</p>
                ...
                <label>Context <span className="optional-label">Optional</span><textarea value={text} onChange={(event) => setText(event.target.value)} rows={3} maxLength={10000} placeholder="Add a note about this file..." /></label>
              </>
            )}
          </div>
          ...
          <div className="form-actions">
            ...
            <PrimaryButton disabled={pending || !canSubmit}>{pending ? "Processing…" : addMode === "file" ? <><FileUp size={16} />Upload File</> : <><Send size={16} />Save Source</>}</PrimaryButton>
          </div>
        ...
        <Panel className="library-browser">
          ...
          <label className="search-field library-search">
            <Search size={16} />
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search titles, authors, and sites" aria-label="Search sources" />
          ...
          <div className="source-list content-swap">
            ...
            {swap === "ready" && !filteredSources.length && <EmptyState title={catalog.pending ? "Searching sources…" : query ? "No matching sources" : "No readings yet"} detail={query ? "Search by title, author, or site." : "Paste an article link or add a document to begin."} />}
